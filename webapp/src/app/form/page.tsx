...
const questions: {
  ...
  },
  {
    id: "riskTolerance",
    title:
      "How would you react if your investment temporarily lost 50% of its value?",
    options: [
...
function QuestionScreen({
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full px-6">
      <h2 className="text-3xl font-bold mb-8 text-gray-800 leading-tight text-center">
        {question.title}
      </h2>
...
function ProfileResult({ answers }: { answers: Record<string, string> }) {
  ...
  const determineProfile = () => {
    ...
    if (
      conservativePoints >= balancedPoints &&
      conservativePoints >= aggressivePoints
    ) {
      return {
        risk: 1,
        profile: "Conservative Investor",
        strategy:
          "We will prioritize safe strategies focusing on education, stablecoins, and staking.",
      };
      ...
      return {
        ...
        strategy:
          "We will implement a balanced strategy with a hint of boldness, including a mix of stablecoins and established cryptocurrencies.",
      };
    } else {
      return {
        risk: 3,
        ...
        strategy:
          "We will pursue an aggressive strategy, including advanced options like yield farming and a focus on high-potential cryptocurrencies.",
      };
    }
  };
  ...
  const handleInvest = async () => {
    await invest(result.risk, getCookie("wallet_address") as string, "1", 600);
  };
            ...
            >
              {isLastStep ? (
                ...
                <QuestionScreen
                  ...
                  onChange={(value) =>
                    handleChange(questions[currentStep].id, value)
                  }
                />
              )}
            </motion.div>
