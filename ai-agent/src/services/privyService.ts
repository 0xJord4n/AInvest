...
import {
  type PrivyPersonalSignResponse,
...
export class PrivyService {
  ...
  async executeTransaction(
    ...
    const response = (await this.makeRequest(
      "/wallets/rpc",
      request
    )) as PrivySignTransactionResponse;
    return response.data.signed_transaction;
  }

  async signMessage(walletAddress: Address, message: string): Promise<string> {
    const request = {
      address: walletAddress,
      chain_type: "ethereum",
